    ...
    async def request_actions() -> None:
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                ...
                files={"root_node": ("index.node.md", root.read_bytes())},
            )
            assert response.status_code == 200
            ...
            response = await client.post(
                ...
                files={
                    "archive": ("fixture.zip", archive.getvalue(), "application/zip")
                },
            )
            assert response.status_code == 202
            response = await client.post(
                "/v1/collections/status", json={"collection_id": collection_id}
            )
            ...
            response = await client.post(
                "/v1/raw/download",
                json={"collection_id": collection_id, "raw_path": source.name},
            )
            assert response.status_code == 200
